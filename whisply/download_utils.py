...
def download_url(url: str, downloads_dir: Path) -> Path:
    ...
    The function first ensures the downloads directory exists, then initiates
    a download using youtube-dl with specific options set for audio quality
    and format. After the download, it extracts the video's information without
    downloading it again to rename the file more meaningfully based on the
    video title. Special characters in the title are replaced with underscores,
    and unnecessary leading or trailing underscores are removed.
    ...
    Returns:
        Path: A path object pointing to the renamed downloaded file. If there
            is an error during the download or file processing, returns None.

    Raises:
        Exception: Outputs an error message to the console if the download
        fails.
