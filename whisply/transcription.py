...
class TranscriptionHandler:
    ...
    def transcribe_with_whisperx(self, filepath: Path) -> dict:
        ...
        word-level timestamps and speaker annotation:
            ...
            for segment in segments:
                ...
                for i, word in enumerate(words):
                    ...
                    if self.annotate and 'speaker' not in word:
                        ...
                        # Case 1: First word: look forward for speaker
                        if i == 0:
                        ...
                        # Case 2: Last word: look backward for last speaker
                        elif i == num_words - 1:
        ...
        transcription_result = help.run_with_progress(
            description=(
                f"[cyan]→ Transcribing "
                f"({'CUDA' if self.device == 'cuda:0' else 'CPU'}) "
    ...
    def transcribe_with_faster_whisper(
        ...
        if self.translate and self.file_language != 'en':
            ...
            translation_chunks = help.run_with_progress(
                description=(
                    f"[dark_blue]→ Translating ({self.device.upper()}) "
                    f"[bold]{filepath.name}"
                ),
